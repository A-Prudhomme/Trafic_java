...
/*        listLight.add(lighttl);
    ...
    public TrafficLight getBottomLight() {
        TrafficLight light_to_return = null;
        for (TrafficLight lamp : light) {
            if (lamp.getPosition() == "bottom-right") {
                light_to_return = lamp;
            }
        }
        return light_to_return;
    }

    public TrafficLight getLeftLight() {
        TrafficLight light_to_return = null;
        for (TrafficLight lamp : light) {
            if (lamp.getPosition() == "bottom-left") {
                light_to_return = lamp;
            }
        }
        return light_to_return;
    ...
    public void change_light(Group root) {
        for (TrafficLight lamp : light) {
            if (lamp.getState().getColor() == Color.GREEN) {
                lamp.setState(new State(Color.ORANGE));
            }
            if (lamp.getState().getColor() == Color.RED) {
                lamp.setState(new State(Color.GREEN));
            }
        }
        for (TrafficLight lamp : light) {
            if (lamp.getState().getColor() == Color.ORANGE) {
                lamp.setState(new State(Color.RED));
            }
        }
        for (TrafficLight light : this.getLight()) {
            light.draw(root);
